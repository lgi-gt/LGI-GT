...
class LGI_GT(nn.Module): 
    ...
    def forward(self, data: Union[Data, Batch]) -> Tensor: 
        ...
        for i in range(self.num_layers // 4): 
            ...
            x = self.gconvs[2*i](x, edge_index, edge_attr) 
            x = self.gconvs[2*i+1](x, edge_index, edge_attr) 
            x = out + x 
            x = self.bns1[i](x) 
            out = x 

            graph = (x, batch) 
            x = self.tlayers[4*i](graph) 
            x = self.tlayers[4*i+1]((x, batch) ) 
            x = self.tlayers[4*i+2]((x, batch) ) 
            x = self.tlayers[4*i+3]((x, batch) ) 
