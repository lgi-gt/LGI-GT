...
class GraphTransformerEncoderLayer(nn.Module): 
    ...
    def __init__(self, hidden_dim, num_heads: int, 
        ...
        self.linear_out1 = nn.Linear(hidden_dim, 4*hidden_dim) 
        self.linear_out2 = nn.Linear(4*hidden_dim, hidden_dim) 
    ...
    def forward(self, graph: Optional[Tuple[Tensor, Tensor, Tensor]]): 
        ...
        # and for clipped nodes (they should not be post-precessed like remain ones, since the former did not pass tlayer while the latter did)
        if batch_CLS is not None: 
