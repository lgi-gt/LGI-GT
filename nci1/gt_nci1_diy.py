...
class GraphTransformer_DIY(Module): 
    ...
    def forward(self, data: Data) -> Tensor: 
        ...
        x = self.gconvs[1](x, edge_index) 
        x = self.bns[1](x) 

        x = x.relu() 
        x = F.dropout(x, p=self.gconv_dropout, training=self.training) 

        x = self.middle_linears[0](x) 
        x = self.pre_lns[0](x) 
        ...
        x = self.gconvs[3](x, edge_index) 
        x = self.bns[3](x) 

        x = x.relu() 
        x = F.dropout(x, p=self.gconv_dropout, training=self.training) 

        x = self.middle_linears[1](x) 
        x = self.pre_lns[1](x) 
        x = F.dropout(x, p=self.between_dropout, training=self.training) 
        ...
        ################# Layer 3 ################
        x = self.gconvs[4](x, edge_index) 
        x = self.bns[4](x) 

        x = x.relu() 
        x = F.dropout(x, p=self.gconv_dropout, training=self.training) 

        x = self.middle_linears[2](x) 
        x = self.pre_lns[2](x) 
        x = F.dropout(x, p=self.between_dropout, training=self.training) 
